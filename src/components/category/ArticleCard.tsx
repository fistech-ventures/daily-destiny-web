...
interface ArticleCardProps {
  article: Article;
  variant?: "horizontal" | "vertical-lead" | "text-only" | "center-featured" | "compact-row";
}
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <h2 
            className={`font-bold text-gray-900 leading-[1.4] group-hover:text-red-600 transition-colors ${
              isCenterFeatured 
          ...
          {isCompactRow && (
            <span className="text-xs text-gray-400 mt-1 block">২ ঘণ্টা আগে</span>
          )}
          {isCenterFeatured && (
            <span className="text-xs text-gray-400 mt-2 block">১ ঘণ্টা আগে</span>
          )}
        </div>
      </div>
    </Link>
